...
* The raw script `Trilinos/checkin-test.py` can still be used for doing local development work and testing or any workflow and can even be used for pushing (but please prefer to use the `checkin-test-sems.sh` script on a RHEL 6 machine with the SEMS env when pushing to the main Trilinos 'develop' branch).  However, unless you can define the `MPI_RELEASE_DEBUG_SHARED_PT.config` file to support the full set of PT packages and TPLs, then just disable that build by running with your own defined builds by passing in `--default-builds= --st-extra-builds=<build0>,<build1>,...`.
